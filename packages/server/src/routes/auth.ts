...
import { clearJwtCookies, extractRefreshToken, setJwtCookies } from '../jwt-cookies'
...
export function buildServer(env: Env) {
  ...
  const credAdapter = adapter as unknown as core.Adapter & {
    ...
    createCredential(userId: core.ID, hash: string): Promise<{ id: core.ID; userId: core.ID }>
  }
  ...
  app.post(
    ...
    async (
      ...
      if (env.SESSION_STRATEGY === 'jwt') {
        ...
        const { user } = await core.login({ email, password }, { adapter: credAdapter, hasher })
        ...
        const { session } = await core.login({ email, password }, { adapter: credAdapter, hasher })
        const cookieOpts = env.COOKIE_DOMAIN ? { domain: env.COOKIE_DOMAIN } : {}
  ...
  app.get('/v1/auth/session', async (req: FastifyRequest, _reply: FastifyReply) => {
    if (env.SESSION_STRATEGY === 'jwt') {
      ...
      try {
        ...
        const { claims } = await verifyJwtFull(accessToken, publicKeys, verifyOpts)
      ...
      const cookie = (req.headers.cookie ?? '')
        ...
        .find((s) => s.startsWith('__keyloom_session='))
      const sid = cookie?.split('=')[1] ?? null
      const { session, user } = await core.getCurrentSession(sid, baseAdapter)
      return {
        session,
