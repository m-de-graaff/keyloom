...
export async function verifyJwtAtEdge(
  ...
  try {
    ...
    if (options.expectedAudience) {
      ...
      const hasValidAudience = expectedAuds.some((expected) =>
        tokenAuds.includes(expected)
      );
  ...
  return async (
    ...
    if (options.verifyAtEdge) {
      ...
      if (options.expectedIssuer !== undefined)
        edgeOpts.expectedIssuer = options.expectedIssuer;
      if (options.expectedAudience !== undefined)
        edgeOpts.expectedAudience = options.expectedAudience;
      if (options.clockSkewSec !== undefined)
        edgeOpts.clockSkewSec = options.clockSkewSec;
      const result = await verifyJwtAtEdge(
        accessToken,
        options.jwksUrl,
        edgeOpts
      );
