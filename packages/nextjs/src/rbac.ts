...
export function getActiveOrgId() {
  return parseCookieValue((headers() as any).get('cookie'), '__keyloom_org')
}

export function setActiveOrgCookie(orgId: string) {
  return `__keyloom_org=${encodeURIComponent(orgId)}; Path=/; SameSite=Lax; HttpOnly; Secure; Max-Age=15552000`
}
