...
function parseArgs(args: string[]) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (typeof a === 'string') rest.push(a)
  }
...
export async function scaffoldCommand(argv: string[]) {
          ...
          await (inquirer as any).prompt([
            {
              ...
              validate: (v: any[]) => (v.length ? true : 'Select at least one page'),
            },
          ])
        ).pages
    ...
    for (const p of pages) {
      switch (p) {
        ...
        case 'forgot-password':
          router === 'app' ? writeAppForgotPassword(cwd) : writePagesForgotPassword(cwd)
          break
        case 'reset-password':
          router === 'app' ? writeAppResetPassword(cwd) : writePagesResetPassword(cwd)
          break
        case 'verify-email':
          router === 'app' ? writeAppVerifyEmail(cwd) : writePagesVerifyEmail(cwd)
          break
        case 'setup-2fa':
  ...
  ui.info('- Add middleware.ts with createAuthMiddleware if not already present')
