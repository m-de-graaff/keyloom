...
function printUsage() {
  ...
  console.log(
    "  migration     Detect adapter and generate RBAC migrations/schema"
  );
...
export function detectAdapter(cwd: string): AdapterKind {
  ...
  if (
    ...
    return "prisma";
  // Raw adapters
  if (hasDep(pkg, "@keyloom/adapters/postgres") || hasDep(pkg, "pg"))
    return "postgres";
  if (hasDep(pkg, "@keyloom/adapters/mysql2") || hasDep(pkg, "mysql2"))
    return "mysql2";
  if (hasDep(pkg, "@keyloom/adapters/mongo") || hasDep(pkg, "mongodb"))
    return "mongo";
  ...
  for (const f of configFiles) {
    try {
      ...
      if (/drizzle/i.test(src))
        return /mysql/i.test(src) ? "drizzle-mysql" : "drizzle-pg";
...
function generatePrisma(cwd: string) {
  ...
  if (!existsSync(schemaPath)) {
    const fullSchema = `// Generated by Keyloom — Prisma schema (default: PostgreSQL)
