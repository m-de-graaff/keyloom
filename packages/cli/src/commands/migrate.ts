...
export async function migrateCommand(args: string[]) {
  ...
  switch (adapter) {
    case 'prisma': {
      const code = await run('npx', ['prisma', 'migrate', 'dev', '--name', 'keyloom_rbac'], cwd)
    ...
    case 'postgres':
    case 'mysql2': {
      console.log('Run your SQL DDL against the database using your preferred tool.')
      break
    }
    case 'mongo': {
      console.log('Run the generated Mongo migration script with your mongo client.')
      break
    }
  }
