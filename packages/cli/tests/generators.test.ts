...
describe("cli generators", () => {
  it("writes config files in expected locations", () => {
    ...
    expect(fs.readFileSync(path.join(dir, "middleware.ts"), "utf8")).toBe("export const config = {}")
  })
  ...
  it("writes next handlers and jwks variants", () => {
    ...
    expect(fs.readFileSync(path.join(dir, "app", "api", "auth", "[[...keyloom]]", "route.ts"), "utf8")).toBe(
