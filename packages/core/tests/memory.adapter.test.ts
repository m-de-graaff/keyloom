...
describe("memory adapter", () => {
  ...
  it("prevents duplicate account links and manages sessions", async () => {
    ...
    await expect(adapter.linkAccount({ ...accountInput(user.id) })).rejects.toThrow(/ACCOUNT_LINKED/)
  ...
  it("handles verification tokens and credentials lifecycle", async () => {
    ...
    await expect((adapter as any).createCredential(user.id, "hash-2"))
      .rejects.toMatchObject({ code: "CREDENTIAL_EXISTS" })
    ...
    await expect((adapter as any).updateCredential(user.id, "hash-4"))
      .rejects.toMatchObject({ code: "CREDENTIAL_NOT_FOUND" })

    await expect((adapter as any).updateCredential("missing", "hash"))
      .rejects.toMatchObject({ code: "CREDENTIAL_NOT_FOUND" })
  })
