...
export function validateKeyloomConfig(config: KeyloomConfig): void {
  ...
  if (
    ...
    !["lax", "strict", "none"].includes(config.cookie.sameSite)
  ) {
    throw new Error(
      'Keyloom config: cookie.sameSite must be "lax", "strict", or "none"'
    );
  }
  ...
  if (config.magicLink?.enabled) {
    if (!config.email) {
      throw new Error(
        "Keyloom config: email configuration is required when magic link is enabled"
      );
    }

    if (
      config.magicLink.defaultTtlMinutes &&
      config.magicLink.defaultTtlMinutes <= 0
    ) {
      throw new Error(
        "Keyloom config: magicLink.defaultTtlMinutes must be positive"
      );
    }

    if (
      config.magicLink.defaultSessionTtlMinutes &&
      config.magicLink.defaultSessionTtlMinutes <= 0
    ) {
      throw new Error(
        "Keyloom config: magicLink.defaultSessionTtlMinutes must be positive"
      );
    }
  }
  ...
  if (config.email) {
    if (!config.email.from) {
      throw new Error("Keyloom config: email.from is required");
    }

    if (!config.email.provider || !config.email.provider.type) {
      throw new Error("Keyloom config: email.provider.type is required");
    }

    if (!["smtp", "resend"].includes(config.email.provider.type)) {
      throw new Error(
        'Keyloom config: email.provider.type must be "smtp" or "resend"'
      );
    }

    if (!config.email.provider.config) {
      throw new Error("Keyloom config: email.provider.config is required");
    }
  }
}
...
function validateJwtConfig(config: KeyloomConfig): void {
  if (!config.jwt) {
    throw new Error(
      'Keyloom config: jwt configuration is required when session.strategy is "jwt"'
    );
  }
  ...
  if (!config.secrets?.authSecret) {
    throw new Error(
      "Keyloom config: secrets.authSecret is required for JWT strategy"
    );
  }

  if (
    config.jwt.algorithm &&
    !["EdDSA", "ES256"].includes(config.jwt.algorithm)
  ) {
  ...
  if (config.jwt.accessTTL && !isValidTTLFormat(config.jwt.accessTTL)) {
    throw new Error(
      'Keyloom config: jwt.accessTTL must be in format like "10m", "1h", "30s"'
    );
  }

  if (config.jwt.refreshTTL && !isValidTTLFormat(config.jwt.refreshTTL)) {
    throw new Error(
      'Keyloom config: jwt.refreshTTL must be in format like "30d", "7d", "24h"'
    );
  }
}
...
export function getConfigFromEnv(): Partial<KeyloomConfig> {
  ...
  // session
  const strategyEnv = process.env.SESSION_STRATEGY as
    | "database"
    | "jwt"
    | undefined;
  ...
  const accessTTL =
    process.env.JWT_ACCESS_TTL || process.env.KEYLOOM_JWT_ACCESS_TTL;
  if (accessTTL !== undefined) jwt.accessTTL = accessTTL;

  const refreshTTL =
    process.env.JWT_REFRESH_TTL || process.env.KEYLOOM_JWT_REFRESH_TTL;
  ...
  if (process.env.JWT_INCLUDE_ORG_ROLE === "true")
    jwt.includeOrgRoleInAccess = true;
  ...
  if (keyRotationDaysStr !== undefined)
    jwt.keyRotationDays = parseInt(keyRotationDaysStr, 10);

  const keyOverlapDaysStr = process.env.KEY_OVERLAP_DAYS;
  if (keyOverlapDaysStr !== undefined)
    jwt.keyOverlapDays = parseInt(keyOverlapDaysStr, 10);
  ...
  const cookieDomain =
    process.env.COOKIE_DOMAIN || process.env.KEYLOOM_COOKIE_DOMAIN;
