...
export async function aesGcmSeal(keyBytes: Uint8Array, plaintext: Uint8Array, aad?: Uint8Array) {
  ...
  const alg = aad ? ({ name: 'AES-GCM', iv, additionalData: aad } as AesGcmParams) : ({ name: 'AES-GCM', iv } as AesGcmParams)
  const ctBuf = await crypto.subtle.encrypt(alg, key, toAb(plaintext))
  return { nonce: Buffer.from(iv).toString('base64url'), ct: Buffer.from(new Uint8Array(ctBuf)).toString('base64url') }
}
