...
export type StandardProviderConfig = {
  ...
  ) => URLSearchParams | Record<string, unknown> | Promise<URLSearchParams | Record<string, unknown>>;
...
export function lintProviderShape(p: Partial<OAuthProvider> & Record<string, any>) {
  ...
  if (style && style !== "json" && style !== "form") errors.push("token.style must be 'json' or 'form'");
