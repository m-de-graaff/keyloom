...
export function prismaAdapter(prisma: AnyPrismaClient): Adapter & {
  ...
  createOrganization?: (data: { name: string; slug?: string | null }) => Promise<any>
  ...
  updateMember?: (id: ID, data: Partial<{ role: string; status: string }>) => Promise<any>
  ...
  // Global Roles extension
  assignGlobalRole?: (data: { userId: ID; role: string }) => Promise<any>
  updateGlobalRole?: (id: ID, data: Partial<{ role: string; status: string }>) => Promise<any>
  removeGlobalRole?: (id: ID) => Promise<void>
  getUserGlobalRole?: (userId: ID) => Promise<any | null>
  listUsersWithGlobalRole?: (role: string) => Promise<any[]>
} {
  return {
    // Users
    ...
    async updateMember(id: ID, data: Partial<{ role: string; status: string }>) {
      const prev = await prisma.membership.findUnique({ where: { id } })
      if (!prev) throw new Error('membership_not_found')
      if (prev.role === 'owner' && typeof data.role !== 'undefined' && data.role !== 'owner') {
      ...
      if (typeof updateData.role !== 'undefined') updateData.role = String(updateData.role).trim()
    ...
    async getEntitlements(orgId: ID) {
      const ent = await prisma.entitlement.findUnique({ where: { orgId } }).catch(() => null)
      return ent ?? null
    },
    ...
    async assignGlobalRole(data: { userId: ID; role: string }) {
      const existing = await prisma.userGlobalRole
        .findUnique({
          where: { userId: data.userId },
        })
        .catch(() => null)
      if (existing) return existing
      const globalRole = await prisma.userGlobalRole.create({
        data: {
          userId: data.userId,
      ...
      try {
        await prisma.auditLog.create({
          data: {
            type: 'global_role.assigned',
            userId: globalRole.userId,
            actorId: null,
            meta: { role: globalRole.role },
          },
        })
      } catch {}
      return globalRole
    },
    async updateGlobalRole(id: ID, data: Partial<{ role: string; status: string }>) {
      const prev = await prisma.userGlobalRole.findUnique({ where: { id } })
      if (!prev) throw new Error('global_role_not_found')
      const updateData: any = { ...data }
      if (typeof updateData.role !== 'undefined') updateData.role = String(updateData.role).trim()
      const globalRole = await prisma.userGlobalRole.update({
        where: { id },
        data: updateData,
      })
      try {
    ...
    async removeGlobalRole(id: ID) {
      const prev = await prisma.userGlobalRole.findUnique({ where: { id } })
      if (!prev) return
      await prisma.userGlobalRole.delete({ where: { id } })
      try {
        await prisma.auditLog.create({
          data: {
            type: 'global_role.removed',
            userId: prev.userId,
            actorId: null,
            meta: { role: prev.role },
          },
    ...
    async getUserGlobalRole(userId: ID) {
      const globalRole = await prisma.userGlobalRole.findUnique({
        where: { userId },
      })
      return globalRole ?? null
    },
    async listUsersWithGlobalRole(role: string) {
      ...
      return globalRoles.map((gr: any) => ({
        ...gr,
        userEmail: (gr as any).user?.email ?? null,
      }))
    },
  }
}
