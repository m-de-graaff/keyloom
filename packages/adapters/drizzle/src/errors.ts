  ...
  mapError(error: unknown): AdapterError | null {
    ...
    if (err.code) {
      switch (err.code) {
        ...
        case '23514': // check_violation
          return new AdapterError(ADAPTER_ERRORS.INVALID_DATA, 'Data validation failed', error)
    ...
    if (err.errno) {
      switch (err.errno) {
        ...
        case 1264: // ER_WARN_DATA_OUT_OF_RANGE
          return new AdapterError(ADAPTER_ERRORS.INVALID_DATA, 'Invalid data provided', error)
    ...
    if (err.message && typeof err.message === 'string') {
      ...
      if (message.includes('not null constraint failed')) {
        return new AdapterError(ADAPTER_ERRORS.INVALID_DATA, 'NOT NULL constraint failed', error)
      }
